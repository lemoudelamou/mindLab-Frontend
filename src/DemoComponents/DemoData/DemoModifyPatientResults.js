...
const DemoModifyPatientResults = () => {
    ...
    const patientId = localStorage.getItem("DemoPatientById");
        ...
        const fetchData = async () => {
            try {
                if (fetchDataTrigger) {
                    ...
                    const response =  await import(`../json/${formattedDemoFullname}.json`);
    ...
    const handleDeleteExperimentData = async (experimentDataId) => {
        ...
        if (confirmDelete) {
            try {
                // Assuming you have a function to delete data from your server (replace with your actual API call)
        ...
        <div className="results-container">
            ...
            {loading ? (
                ...
                <div className="pad-container">
                    ...
                    <h3>Patient Information: {experimentDetails[0].patient.fullname}</h3>
                    <table>
                        <tbody>
                        <tr>
                            <td>Fullname:</td>
                            <td>{experimentDetails[0].patient.fullname}</td>
                        </tr>
                        <tr>
                            <td>Birth Date:</td>
                            <td>{experimentDetails[0].patient.birthDate ? new Date(experimentDetails[0].patient.birthDate).toLocaleDateString() : 'N/A'}</td>
                        </tr>
                        <tr>
                            <td>Age:</td>
                        ...
                        <tr>
                            <td>Strong Hand:</td>
                            <td>{experimentDetails[0].patient.strongHand}</td>
                        </tr>
                        <tr>
                            <td>Group:</td>
                            <td>{experimentDetails[0].patient.groupe}</td>
                        </tr>
                        <tr>
                            <td>Has Diseases:</td>
                            <td>{experimentDetails[0].patient.hasDiseases  ? 'Yes' : 'No'}</td>
                        </tr>
                        {experimentDetails[0].patient.hasDiseases && (
                            <tr>
                                <td>Diseases:</td>
                                <td>{experimentDetails[0].patient.diseases}</td>
                            </tr>
                        )}
                        <tr>
                            <td>Experiment datte:</td>
                            <td>{experimentDetails[0].patient.expDate}</td>
                        </tr>
                        </tbody>
                    </table>
